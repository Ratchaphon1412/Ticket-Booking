...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchData = async () => {
      try {
        const token =
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MiwiaWF0IjoxNjg0OTAxNDQzLCJleHAiOjE3NzEzMDE0NDN9.YehEuxm0-1NHrn-8XwufivF2aYoU-dyMkCDfqeHJbAM";
        const options = {
          headers: {
        ...
        const response = await fetch(
          "https://sore-leg-warmers-clam.cyclic.app/api/concert",
          options
        );
    ...
    const deleteProduct = async (id: number) => {
      ...
      try {
        ...
        if (result.isConfirmed) {
          const response = await fetch(
            `https://sore-leg-warmers-clam.cyclic.app/api/admin/concert/delete/${id}`,
            options
          );
          if (response.ok) {
